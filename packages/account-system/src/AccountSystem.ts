...
export class AccountSystemLengthError extends Error {
  constructor(item: string, min: number, max: number, recieved: number) {
    super(
      `AccountSystemLengthError: Invalid length of "${item}". Expected between ${min} and ${max}. Got ${recieved}`
    );
  }
}
...
export class AccountSystemNotEmptyError extends Error {
  constructor(type: string, path: string, action: string) {
    super(
      `AccountSystemNotEmptyError: ${type} "${path}" must be empty to ${action}`
    );
  }
}
...
const validateFilename = (name: string) => {
  ...
  if (name.length < 1 || name.length > 255) {
    throw new AccountSystemLengthError(
      `filename ("${name}")`,
      1,
      255,
      name.length
  ...
  for (let dir of path.split(posix.sep).slice(1)) {
    try {
      ...
      if (err instanceof AccountSystemLengthError) {
        throw new AccountSystemLengthError(
          `directory ("${dir}" of "${path}")`,
          1,
          255,
          dir.length
        );
      } else if (err instanceof AccountSystemSanitizationError) {
        throw new AccountSystemSanitizationError("directory", dir, [
          posix.sep,
          "\0",
        ]);
      } else {
...
const unfreezeFileMetadata = (
  doc: Automerge.FreezeObject<FileMetadata>
): FileMetadata => {
  return {
    ...
    private: {
      handle: doc?.private?.handle
        ? unfreezeUint8Array(doc.private.handle)
        : null,
    },
    public: {
      location: doc?.public?.location
        ? unfreezeUint8Array(doc.public.location)
        : null,
...
const unfreezeFolderMetadata = (
  doc: Automerge.FreezeObject<FolderMetadata>
): FolderMetadata => {
  return {
    location: unfreezeUint8Array(doc.location),
    name: doc.name,
...
const unfreezeShareMetadata = (
  doc: Automerge.FreezeObject<ShareMetadata>
): ShareMetadata => {
  return {
    ...
    files: doc.files.map((file) => ({
      ...
      private: {
        handle: file?.private?.handle
          ? unfreezeUint8Array(file.private.handle)
          : null,
      },
      public: {
        location: file?.public?.location
          ? unfreezeUint8Array(file.public.location)
          : null,
      },
    })),
  };
...
export class AccountSystem {
  ...
  async getFilesIndex(markCacheDirty = false): Promise<FilesIndex> {
    ...
    return await this._m.runExclusive(() =>
      this._getFilesIndex(markCacheDirty)
    );
  }
  ...
  async _getFilesIndex(markCacheDirty = false): Promise<FilesIndex> {
    ...
    const filesIndex =
      (await this.config.metadataAccess.get<FilesIndex>(
        this.indexes.files,
        markCacheDirty
  ...
  async getFileMetadataLocationByFileHandle(
    ...
    return await this._m.runExclusive(() =>
      this._getFileMetadataLocationByFileHandle(fileHandle, markCacheDirty)
    );
  }

  async _getFileMetadataLocationByFileHandle(
    fileHandle: Uint8Array,
    markCacheDirty = false
  ): Promise<Uint8Array> {
    ...
    const fileEntry = filesIndex.files.find(
      (file) =>
        file.private.handle && arraysEqual(file.private.handle, fileHandle)
    );

    if (!fileEntry) {
      throw new AccountSystemNotFoundError(
        "file of handle",
        bytesToHex(fileHandle.slice(0, 32)) + "..."
      );
    }
  ...
  async getFileMetadataLocationByFileLocation(
    ...
    return await this._m.runExclusive(() =>
      this._getFileMetadataLocationByFileHandle(fileLocation, markCacheDirty)
    );
  }

  async _getFileMetadataLocationByFileLocation(
    fileLocation: Uint8Array,
    markCacheDirty = false
  ): Promise<Uint8Array> {
    ...
    const fileEntry = filesIndex.files.find(
      (file) =>
        file.public.location && arraysEqual(file.public.location, fileLocation)
    );

    if (!fileEntry) {
      throw new AccountSystemNotFoundError(
        "file of handle",
        bytesToHex(fileLocation.slice(0, 32)) + "..."
      );
    }
  ...
  async getFileIndexEntryByFileMetadataLocation(
    ...
    return await this._m.runExclusive(() =>
      this._getFileIndexEntryByFileMetadataLocation(location, markCacheDirty)
    );
  }
  ...
  async _getFileIndexEntryByFileMetadataLocation(
    ...
    const fileEntry = filesIndex.files.find((file) =>
      arraysEqual(file.location, location)
    );
  ...
  async getFileMetadata(
    ...
    return await this._m.runExclusive(() =>
      this._getFileMetadata(location, markCacheDirty)
    );
  }
  ...
  async _getFileMetadata(
    ...
    const doc = await this.config.metadataAccess.get<FileMetadata>(
      filePath,
      markCacheDirty
    );
  ...
  async _addUpload(
    ...
    const fileHandle = fileEncryptionKey
      ? arrayMerge(fileLocation, fileEncryptionKey)
      : fileLocation;
  ...
  async finishUpload(
    ...
    return await this._m.runExclusive(() =>
      this._finishUpload(location, markCacheDirty)
    );
  }
  ...
  async _finishUpload(
    ...
    await this.config.metadataAccess.change<FilesIndex>(
      ...
      (doc) => {
        const fileEntry = doc.files.find((file) =>
          arraysEqual(location, file.location)
        );

        if (!fileEntry) {
          throw new AccountSystemNotFoundError(
            "file entry",
            `"${bytesToB64URL(location)}" in "${bytesToB64URL(
              unfreezeUint8Array(fileMeta.folderDerive)
            )}"`
          );
        }
  ...
  async setFilePrivateHandle(
    ...
    return await this._m.runExclusive(() =>
      this._setFilePrivateHandle(location, newFileHandle, markCacheDirty)
    );
  }
  ...
  async _setFilePrivateHandle(
    ...
    const filesIndex = await this._getFilesIndex(markCacheDirty);
    const fileEntry = filesIndex.files.find((fileEntry) =>
      arraysEqual(fileEntry.location, location)
    );

    if (!fileEntry) {
      throw new AccountSystemNotFoundError(
        "file entry",
        bytesToB64URL(location)
      );
    }
    ...
    await this.config.metadataAccess.change<FilesIndex>(
      ...
      (doc) => {
        const entryIndex = doc.files.findIndex((fileEntry) =>
          arraysEqual(fileEntry.location, location)
        );
  ...
  async setFilePublicLocation(
    ...
    return await this._m.runExclusive(() =>
      this._setFilePublicLocation(location, newFileLocation, markCacheDirty)
    );
  }
  ...
  async addFilePublicShortlink(
    ...
    return await this._m.runExclusive(() =>
      this._addFilePublicShortlink(location, shortlink, markCacheDirty)
    );
  }
  ...
  async removeFilePublicShortlink(
    ...
    return await this._m.runExclusive(() =>
      this._removeFilePublicShortlink(location, shortlink, markCacheDirty)
    );
  }
  ...
  async renameFile(
    ...
    return await this._m.runExclusive(() =>
      this._renameFile(location, newName, markCacheDirty)
    );
  }
  ...
  async _renameFile(
    ...
    const fileIndexEntry = await this._getFileIndexEntryByFileMetadataLocation(
      location,
      markCacheDirty
    );
    if (!fileIndexEntry) {
  ...
  async moveFile(
    ...
    return await this._m.runExclusive(() =>
      this._moveFile(location, newPath, markCacheDirty)
    );
  }
  ...
  async _moveFile(
    ...
    await this.config.metadataAccess.change<FolderMetadata>(
      ...
      `Move file ${bytesToB64URL(location)}`,
      (doc) => {
        const fileEntryIndex = doc.files.findIndex((file) =>
          arraysEqual(location, file.location)
        );
  ...
  async removeFile(location: Uint8Array, markCacheDirty = false) {
    ...
    return await this._m.runExclusive(() =>
      this._removeFile(location, markCacheDirty)
    );
  }
  ...
  async _removeFile(location: Uint8Array, markCacheDirty = false) {
    ...
    await this.config.metadataAccess.change<FilesIndex>(
      ...
      (doc) => {
        const fileEntry = doc.files.find((file) =>
          arraysEqual(unfreezeUint8Array(file.location), location)
        );
    ...
    await this.config.metadataAccess.change<FolderMetadata>(
      ...
      (doc) => {
        const fileIndex = doc.files.findIndex((file) =>
          arraysEqual(unfreezeUint8Array(file.location), location)
        );
  ...
  async getFoldersIndex(markCacheDirty = false): Promise<FoldersIndex> {
    ...
    return await this._m.runExclusive(() =>
      this._getFoldersIndex(markCacheDirty)
    );
  }
  ...
  async _getFoldersIndex(markCacheDirty = false): Promise<FoldersIndex> {
    ...
    const foldersIndex =
      (await this.config.metadataAccess.get<FoldersIndex>(
  ...
  async getFolderIndexEntryByPath(
    path: string,
    markCacheDirty = false
    ...
    return await this._m.runExclusive(() =>
      this._getFolderIndexEntryByPath(path, markCacheDirty)
    );
  }

  async _getFolderIndexEntryByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry> {
    ...
    const folderEntry = foldersIndex.folders.find(
  ...
  async getFoldersInFolderByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry[]> {
    ...
    return await this._m.runExclusive(() =>
      this._getFoldersInFolderByPath(path, markCacheDirty)
    );
  }

  async _getFoldersInFolderByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry[]> {
    ...
    return foldersIndex.folders.filter((folder) =>
      isPathChild(path, folder.path)
    );
  }

  async getAllFoldersInFolderRecursivelyByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry[]> {
    ...
    return await this._m.runExclusive(() =>
      this._getAllFoldersInFolderRecursivelyByPath(path, markCacheDirty)
    );
  }

  async _getAllFoldersInFolderRecursivelyByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry[]> {
      ...
      await Promise.all(
        foldersIndex.folders
          ...
          .map(async (folder) =>
            [
              folder,
              await this._getAllFoldersInFolderRecursivelyByPath(folder.path),
            ].flat()
          )
      )
    ).flat();
  ...
  async getAllFilesInFolderRecursivelyByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FilesIndexEntry[]> {
    // console.log("getAllFoldersInFolderRecursivelyByPath(", path, ")")
    ...
    return await this._m.runExclusive(() =>
      this._getAllFilesInFolderRecursivelyByPath(path, markCacheDirty)
    );
  }

  async _getAllFilesInFolderRecursivelyByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FilesIndexEntry[]> {
    // console.log("_getAllFoldersInFolderRecursivelyByPath(", path, ")")
    ...
    const foldersInFolder = await this._getAllFoldersInFolderRecursivelyByPath(
      path,
      markCacheDirty
    );
    ...
    const filesInFolder = (
      await Promise.all(
        foldersInFolder.map(
          async (folder) =>
            (
              await this._getFolderMetadataByPath(folder.path, markCacheDirty)
            ).files
        )
      )
    ).flat();
    ...
    return filesIndex.files.filter((fileEntry) =>
      ([] as FolderFileEntry[])
        ...
        .some((folderFileEntry) =>
          arraysEqual(folderFileEntry.location, fileEntry.location)
        )
    );
  }

  async getFoldersInFolderByLocation(
    location: Uint8Array,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry[]> {
    ...
    return await this._m.runExclusive(() =>
      this._getFoldersInFolderByLocation(location, markCacheDirty)
    );
  }

  async _getFoldersInFolderByLocation(
    location: Uint8Array,
    markCacheDirty = false
  ): Promise<FoldersIndexEntry[]> {
    ...
    const folderEntry = foldersIndex.folders.find((folder) =>
      arraysEqual(folder.location, location)
    );
  ...
  async getAllFoldersInFolderRecursivelyByLocation(
    ...
    return await this._m.runExclusive(() =>
      this._getAllFoldersInFolderRecursivelyByLocation(location, markCacheDirty)
    );
  }
      ...
      await Promise.all(
        foldersIndex.folders
          ...
          .map(async (folder) =>
            [
              folder,
              await this._getAllFoldersInFolderRecursivelyByLocation(
                folder.location
              ),
            ].flat()
          )
  ...
  async getAllFilesInFolderRecursivelyByLocation(
    ...
    return await this._m.runExclusive(() =>
      this._getAllFilesInFolderRecursivelyByLocation(location, markCacheDirty)
    );
  }
    ...
    const filesInFolder = (
      await Promise.all(
        foldersInFolder.map(
          async (folder) =>
            (
              await this._getFolderMetadataByLocation(
                folder.location,
                markCacheDirty
              )
            ).files
  ...
  async getFolderMetadataByPath(
    ...
    return await this._m.runExclusive(() =>
      this._getFolderMetadataByPath(path, markCacheDirty)
    );
  }

  async _getFolderMetadataByPath(
    path: string,
    markCacheDirty = false
  ): Promise<FolderMetadata> {
    ...
    const folderEntry = await this._getFolderIndexEntryByPath(
      path,
      markCacheDirty
    );

    return await this._getFolderMetadataByLocation(
      folderEntry.location,
      markCacheDirty
    );
  }

  async getFolderMetadataByLocation(
    location: Uint8Array,
    markCacheDirty = false
  ): Promise<FolderMetadata> {
    ...
    return await this._m.runExclusive(() =>
      this._getFolderMetadataByLocation(location, markCacheDirty)
    );
  }

  async _getFolderMetadataByLocation(
    location: Uint8Array,
    markCacheDirty = false
  ): Promise<FolderMetadata> {
    ...
    const doc = await this.config.metadataAccess.get<FolderMetadata>(
      folderPath,
      markCacheDirty
    );
  ...
  async addFolder(
    ...
    return await this._m.runExclusive(() =>
      this._addFolder(path, markCacheDirty)
    );
  }
  ...
  async renameFolder(
    ...
    return await this._m.runExclusive(() =>
      this._renameFolder(path, newName, markCacheDirty)
    );
  }
  ...
  async _renameFolder(
    ...
    return await this._moveFolder(
      path,
      posix.join(posix.dirname(path), newName),
      markCacheDirty
    );
  }
  ...
  async moveFolder(
    ...
    return await this._m.runExclusive(() =>
      this._moveFolder(oldPath, newPath, markCacheDirty)
    );
  }
  ...
  async _moveFolder(
    ...
    const op =
      posix.dirname(oldPath) == posix.dirname(newPath) ? "Rename" : "Move";

    const newFolder = await this._getFolderIndexEntryByPath(
      newPath,
      markCacheDirty
    ).catch(() => {});
    if (newFolder) {
    ...
    await this.config.metadataAccess.change<FoldersIndex>(
      ...
      `${op} folder`,
      (doc) => {
        const subs = doc.folders.filter(
          (folderEntry) =>
            posix.relative(oldPath, folderEntry.path).indexOf("../") != 0
        );

        for (let folderEntry of subs) {
          folderEntry.path = posix.join(
            newPath,
            posix.relative(oldPath, folderEntry.path)
          );
        }
      },
      markCacheDirty
  ...
  async removeFolderByPath(
    ...
    return await this._m.runExclusive(() =>
      this._removeFolderByPath(path, markCacheDirty)
    );
  }

  async _removeFolderByPath(
    path: string,
    markCacheDirty = false
  ): Promise<void> {
    ...
    return await this._removeFolderByLocation(
      folderEntry.location,
      markCacheDirty
    );
  }

  async removeFolderByLocation(
    location: Uint8Array,
    markCacheDirty = false
  ): Promise<void> {
    ...
    return await this._m.runExclusive(() =>
      this._removeFolderByLocation(location, markCacheDirty)
    );
  }

  async _removeFolderByLocation(
    location: Uint8Array,
    markCacheDirty = false
  ): Promise<void> {
    // console.log("_removeFolderByLocation(", location, ")")

    const folderMeta = await this._getFolderMetadataByLocation(
      location,
      markCacheDirty
    );

    if (folderMeta.files.length) {
      throw new AccountSystemNotEmptyError(
        "folder",
        bytesToB64URL(location),
        "remove"
      );
    ...
    const childFolders = await this._getFoldersInFolderByLocation(
      location,
      markCacheDirty
    );
    await Promise.all(
      childFolders.map((folder) =>
        this._removeFolderByLocation(folder.location)
      )
    );
    ...
    await this.config.metadataAccess.change<FoldersIndex>(
      ...
      (doc) => {
        const folderIndex = doc.folders.findIndex((file) =>
          arraysEqual(unfreezeUint8Array(file.location), location)
        );
  ...
  async getShareIndex(markCacheDirty = false): Promise<ShareIndex> {
    ...
    return await this._m.runExclusive(() =>
      this._getShareIndex(markCacheDirty)
    );
  }
  ...
  async _getShareIndex(markCacheDirty = false): Promise<ShareIndex> {
    ...
    const sharedIndex =
      (await this.config.metadataAccess.get<ShareIndex>(
        this.indexes.share,
        markCacheDirty
    ...
    return {
      shared: sharedIndex.shared.map((shareEntry) => ({
        ...
        fileLocations: shareEntry.fileLocations.map((l) =>
          unfreezeUint8Array(l)
        ),
      })),
    };
  }
  ...
  async getSharesByHandle(handle: Uint8Array, markCacheDirty = false) {
    ...
    return await this._m.runExclusive(() =>
      this._getSharesByHandle(handle, markCacheDirty)
    );
  }

  async _getSharesByHandle(
    handle: Uint8Array,
    markCacheDirty = false
    ...
    return shareIndex.shared.filter(
      (share) =>
        share.fileHandles.findIndex((h) => arraysEqual(handle, h)) != -1
    );
  }

  async share(
    filesInit: ShareFileMetadataInit[],
    markCacheDirty = false
  ): Promise<ShareMetadata> {
    // console.log("share(", filesInit, ")")

    return await this._m.runExclusive(() =>
      this._share(filesInit, markCacheDirty)
    );
  }
  ...
  async _share(
    ...
    const files = await Promise.all(
      filesInit.map(async (fileInit): Promise<ShareFileMetadata> => {
        const meta = await this._getFileMetadata(
          fileInit.location,
          markCacheDirty
        );
    ...
    const shareMeta =
      await this.config.metadataAccess.changePublic<ShareMetadata>(
        locationKey,
        "Share files",
        (doc) => {
