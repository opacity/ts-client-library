...
export class Account {
  ...
  async createSubscription({
    stripeToken = "",
  }: AccountStripeArgs = {}): Promise<AccountGetRes> {
    const payload = await getPayload({
      crypto: this.config.crypto,
      payload: { stripeToken },
    });
    ...
    if (!res.ok) {
      throw new Error(
        "Error getting Stripe information: " + JSON.stringify(res.data)
      );
    }
  ...
  async plans() {
    ...
    const plans: AccountPlanInfo[] = Object.values(res.data.plans).filter(
      (plan) => plan.storageInGB <= 2048
    );
  ...
  async signUp({
    size = 128,
    duration = 12,
  }: AccountSignupArgs = {}): Promise<AccountCreationInvoice> {
    try {
      const info = await this.info();
  ...
  async renewStatus({
    fileIDs,
    metadataKeys,
  }: AccountRenewStatusArgs): Promise<AccountRenewStatus> {
  ...
  async renewAccount({
    duration = 12,
  }: AccountRenewArgs): Promise<AccountRenewInvoice> {
    try {
      const info = await this.info();
  ...
  async waitForRenewPayment(
    renewStatusArgs: AccountRenewStatusArgs
  ): Promise<void> {
    const [done, resolveDone] = extractPromise<void>();
  ...
  async upgradeStatus({
    fileIDs,
    metadataKeys,
  }: AccountUpgradeStatusArgs): Promise<AccountUpgradeStatus> {
  ...
  async upgradeAccount({
    size,
    duration = 12,
  }: AccountUpgradeArgs): Promise<AccountUpgradeInvoice> {
    try {
      const info = await this.info();
  ...
  async waitForUpgradePayment(
    UpgradeStatusArgs: AccountUpgradeStatusArgs
  ): Promise<void> {
    const [done, resolveDone] = extractPromise<void>();
